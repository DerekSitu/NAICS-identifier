...
class NAICSCodeAssigner:
    ...
    '''
    Class for assigning a suitable NAICS code to a business.
    ...
    def __init__(self, input_filename, separator_input_file, model,
                 output_dir, output_filename, columns_business_description,
                 prompt_template=None, num_digits_naics_code=4,
                 max_retries=3, min_wait_time=2, chunk_size=100):
        ...
        if output_filename is not None:
            ...
            self.output_filename = self.output_dir + \
                '/' + f'companies_naics.csv'

        if prompt_template is not None:
            self.prompt_template = prompt_template
        else:
            self.prompt_template = ''' \
            Assign the closest 4-digit 2022 NAICS code to this business. \
            List only the 4-digit 2022 NAICS code, and no other text. \
            For example, for the 2022 NAICS category \
        ...
        if columns_business_description is not None:
            ...
            self.columns_business_description = ['Company', 'Snippet']

        self.progress_log = self.output_dir + \
                '/' + f'progress_naics.txt'
    ...
    def get_completion(self, prompt, model='gpt-4o-mini'):
        ...
        model : str
            GPT model being used. The default is 'gpt-4o-mini'.
